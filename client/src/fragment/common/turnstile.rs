                ...
                spawn_local({
                    ...
                    async move {
                        match TURNSTILE_LOADER.get_or_load().await {
                            Ok(turnstile) => {
                                let render_method: js_sys::Function =
                                ...
                                let callback =
                                    Closure::wrap(Box::new(move |token: JsValue| -> () {
                                        if let Some(token) = token.as_string() {
                                            ondone.run(TokenResult::Success(SharedString::from(
                                                token,
                                            )));
                                        } else {
                                            log::error!(
                                ...
                                let error_callback =
                                    Closure::wrap(Box::new(move |error_code: JsValue| -> () {
                                        log::error!("turnstile校验失败，错误码: {:?}", error_code);
                                        ondone.run(TokenResult::Failure(SharedString::from(
                                            "环境检测未通过!",
                                        )));
                                    })
                            ...
                            Err(error) => {
                                ...
                                ondone
                                    .run(TokenResult::Failure(SharedString::from("环境检测失败!")));
                            }
                        }
                    }
                });
