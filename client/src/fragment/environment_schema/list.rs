...
#[function_component]
pub fn EnvironmentSchemaList() -> Html {
    let pagination: UseStateHandle<Pagination> = use_state(|| Pagination::new(0, 1, None, None));
    let list: UseStateHandle<Vec<EnvironmentSchema>> = use_state(|| Vec::new());
    ...
    html! {
        <div class="relative width-fill height-fill" style="overflow:hidden;">
            ...
            <Drawer active={*edit_active} onclickother={on_leave_edit}>
                <EnvironmentSchemaEdit id={active_edit_id.deref().clone()} onsave={on_finish_save} />
            </Drawer>
        </div>
    }
}
                        ...
                        <tbody>
                            {
                                for list.iter().map(|item| {
                                    row_view(list, list_load_status, pagination, item, detail_active, active_detail_id, edit_active, active_edit_id)
                                })
                            }
                        </tbody>
                    }
...
fn row_view(
    ...
    return html! {
        <tr class="e-table-row">
            <td class="e-table-cell align-center">{&environment_schema.name}</td>
...
async fn refresh_list(
    ...
    query_environment_schema_list(page_no.max(1), list, list_load_status, pagination).await;
}
...
fn remove_environment_schema(id: Id, list: &UseStateHandle<Vec<EnvironmentSchema>>) {
