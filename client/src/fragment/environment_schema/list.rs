use super::detail::EnvironmentSchemaDetail;
use super::edit::EnvironmentSchemaEdit;
...
#[component]
pub fn EnvironmentSchemaList() -> impl IntoView {
    let pagination: RwSignal<Pagination> = RwSignal::new(Pagination::new(0, 1, None, None));
    let list: RwSignal<Vec<EnvironmentSchema>> = RwSignal::new(Vec::new());
    ...
    let query_by_page = move |page: u64| {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            query_environment_schema_list(page, &list, &list_load_status, &pagination).await;
        });
    };
    let on_query = {
    ...
    let on_finish_save = UnsyncCallback::new(move |_pri_key: PrimaryKey| {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            if active_edit_id.read().is_some() {
                ...
                query_environment_schema_list(1, &list, &list_load_status, &pagination).await;
            }
        });
    });
                ...
                <div class="width-fill height-fill" style="display: -webkit-box;display: flex;-webkit-box-direction: normal;-webkit-box-orient: vertical;flex-direction: column;">
                    <header style="-webkit-box-flex: 0;flex-basis: auto;flex-grow: 0;">
                        <div style="display:flex;justify-content: space-between;padding-bottom: 0.25em;">
                            <span style="font-weight:bold;">{"环境规格列表"}</span>
                            <ButtonGroup>
                    ...
                    <div style="-webkit-box-flex: 1;flex-basis: auto;flex-grow: 1;overflow-y: auto;">
                        { table_view(&list, &list_load_status, &pagination,&detail_active, &active_detail_id, &edit_active, &active_edit_id) }
                    ...
                    move || {
                        match active_detail_id.get() {
                            Some(active_detail_id) => view! {
                                <EnvironmentSchemaDetail id={active_detail_id} />
                            }.into_any(),
                            None => view! {}.into_any()
                        }
                    }
...
fn table_view(
    ...
    return view! {
        <table class="e-table width-fill">
            <thead>
                <tr>
                    <th class="e-table-hcell">{"环境规格名称"}</th>
            ...
            <Show when=move || {
                ...
                <tbody>
                    <For
                        ...
                        children=move |item| {
                            row_view(&list.read_value(), &list_load_status.read_value(), &pagination.read_value(), &item, &detail_active.read_value(), &active_detail_id.read_value(), &edit_active.read_value(), &active_edit_id.read_value())
                        }
                    />
                </tbody>
            </Show>
...
fn row_view(
    ...
    let detail_id = environment_schema.id;
    ...
    let update_id = environment_schema.id;
    ...
    let delete_id = environment_schema.id;
    let list = list.clone();
    ...
    let on_confirm_delete = UnsyncCallback::new(move |_: ()| {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            ...
            if ok {
                ...
                let list_load_status = list_load_status.clone();
                let pagination = pagination.clone();
                delete_environment_schema(delete_id, &list, &list_load_status, &pagination)
                    .await
                    .ok();
            }
        });
    ...
    return view! {
        <tr class="e-table-row">
            <td class="e-table-cell align-center">{environment_schema.name.clone()}</td>
...
fn clear_list(list: &RwSignal<Vec<EnvironmentSchema>>, pagination: &RwSignal<Pagination>) {
...
async fn refresh_list(
    ...
    query_environment_schema_list(new_page_no, list, list_load_status, pagination).await;
}

async fn delete_environment_schema(
    id: Id,
    list: &RwSignal<Vec<EnvironmentSchema>>,
    list_load_status: &RwSignal<LoadStatus>,
    pagination: &RwSignal<Pagination>,
...
fn remove_environment_schema(id: Id, list: &RwSignal<Vec<EnvironmentSchema>>) {
