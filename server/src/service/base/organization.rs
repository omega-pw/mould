...
use crate::log;
use crate::model::organization::properties;
use crate::model::organization::Organization;
use crate::model::organization::OrganizationProperty;
use crate::model::organization::OrganizationOpt;
use crate::native_common;

const ENTITY: &str = "organization";
const EXTRA_PROPERTIES: [&str; 3] = [properties::NAME,properties::CREATED_TIME,properties::LAST_MODIFIED_TIME,];
...
fn extract_organization(row: &Row) -> Result<Organization, ErrNo> {
...
fn opt_to_conditions<'a>(opt: &'a OrganizationOpt) -> Vec::<(Condition, &'a (dyn ToSql + std::marker::Sync))> {
...
pub struct OrganizationBaseService<'a> {
    transaction: &'a Transaction<'a>
}

impl<'a> OrganizationBaseService<'a> {

    pub fn new(transaction: &'a Transaction) -> OrganizationBaseService<'a> {
        return OrganizationBaseService {
            transaction: transaction
        };
    }

    pub async fn read_organization(&self, id: Id) -> Result<Option<Organization>, ErrNo> {
        ...
        return Ok(row_opt.as_ref().map(extract_organization).transpose()?);
    }

    pub async fn read_organization_batch(&self, ids: &[Id]) -> Result<Vec<Organization>, ErrNo>
    {
        if ids.is_empty() {
            log::warn!("待批量读取的组织id集合为空");
            return Ok(Vec::new());
        }
        ...
        let list: Vec<Organization> = rows.iter().map(extract_organization).collect::<Result<Vec<_>, _>>()?;
        return Ok(list);
    }

    pub async fn insert_organization(&self, organization: &Organization) -> Result<(), ErrNo> {
        let mut vals: Vec<&(dyn ToSql + std::marker::Sync)> = Vec::with_capacity(PROPERTY_COUNT);
        let sql = format_xml::template! {
            "insert into "{ENTITY}"(" {PROPERTIES.as_str()} ") values(" {
                vec![
                    add_val(&mut vals, &organization.id),
                    add_val(&mut vals, &organization.name),
                    add_val(&mut vals, &organization.created_time),
                    add_val(&mut vals, &organization.last_modified_time),
                ].join(",")
            } ")"
        }.to_string();
        let statement = self.transaction.prepare(&sql).await.map_err(prepare_statement_error)?;
    ...
    pub async fn insert_organization_batch(&self, organization_list: &[Organization]) -> Result<(), ErrNo> {
        if organization_list.is_empty() {
            log::warn!("待批量新增的组织集合为空");
            return Ok(());
        }
        let mut vals: Vec<&(dyn ToSql + std::marker::Sync)> = Vec::with_capacity(PROPERTY_COUNT * organization_list.len());
        let sql = format_xml::template! {
            "insert into "{ENTITY}"(" {PROPERTIES.as_str()} ") values" {
                organization_list.iter().map(|organization|{
    ...
    pub async fn update_organization_full(&self, organization: &Organization) -> Result<(), ErrNo> {
        let mut vals: Vec<&(dyn ToSql + std::marker::Sync)> = Vec::with_capacity(PROPERTY_COUNT);
        let sql = format_xml::template! {
            "update "{ENTITY}" set " {
                [
                    properties::NAME, "=", &add_val(&mut vals, &organization.name),
                    ",", properties::CREATED_TIME, "=", &add_val(&mut vals, &organization.created_time),
                    ",", properties::LAST_MODIFIED_TIME, "=", &add_val(&mut vals, &organization.last_modified_time),
                ].concat()
            } " where "{properties::ID}"=" {add_val(&mut vals, &organization.id)}
        }.to_string();
        let statement = self.transaction.prepare(&sql).await.map_err(prepare_statement_error)?;
        self.transaction.execute(&statement, &vals).await.map_err(execute_error)?;
        return Ok(());
    ...
    pub async fn update_organization(&self, id: Id, changes: &[OrganizationProperty]) -> Result<(), ErrNo> {
        let changes: Vec<&OrganizationProperty> = changes
            .iter()
            .filter(|change| match change {
                OrganizationProperty::Id(_) => false,
                _ => true,
            })
            .collect();
        if changes.is_empty() {
    ...
    pub async fn delete_organization(&self, id: Id) -> Result<(), ErrNo> {
        let mut vals: Vec<&(dyn ToSql + std::marker::Sync)> = Vec::with_capacity(1);
        let sql = format_xml::template! {
    ...
    pub async fn delete_organization_batch(&self, ids: &[Id]) -> Result<(), ErrNo>
    {
        if ids.is_empty() {
            log::warn!("待批量删除的组织id集合为空");
            return Ok(());
        }
        let mut vals: Vec<&(dyn ToSql + std::marker::Sync)> = Vec::with_capacity(ids.len());
        let sql = format_xml::template! {
    ...
    pub async fn query_organization_count(&self, opt: &OrganizationOpt) -> Result<u64, ErrNo> {
    ...
    pub async fn query_organization(&self, page_no: u64, page_size: u64, opt: &OrganizationOpt) -> Result<Vec<Organization>, ErrNo> {
        let pairs = opt_to_conditions(opt);
        let mut vals: Vec<&(dyn ToSql + std::marker::Sync)> = Vec::with_capacity(pairs.len());
    ...
    pub async fn query_organization_one(&self, opt: &OrganizationOpt) -> Result<Option<Organization>, ErrNo> {
        let list = self.query_organization(1, 1, opt).await?;
        return Ok(list.into_iter().next());
    }

    pub async fn query_organization_batch(&self, opt: &OrganizationOpt) -> Result<Vec<Organization>, ErrNo> {
        let pairs = opt_to_conditions(opt);
        let mut vals: Vec<&(dyn ToSql + std::marker::Sync)> = Vec::with_capacity(pairs.len());
        let sql = format_xml::template! {
            "select "{PROPERTIES.as_str()}" from "{ENTITY}
