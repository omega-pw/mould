...
async fn try_handle(
    ...
    append_log(LogLevel::Info, String::from("正在连接postgresql服务器"));
    let client = get_client(configuration, root_cert).await?;
    append_log(LogLevel::Info, String::from("连接postgresql服务器成功"));
